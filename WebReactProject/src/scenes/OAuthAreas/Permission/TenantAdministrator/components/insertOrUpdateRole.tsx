...
export interface IInsertOrUpdateRole {
    location: any;
    value: RoleTenantReadDto | undefined;
    idTenant: number;
    onClose: () => void;
}

export default function InsertOrUpdateRole(props: IInsertOrUpdateRole) {
    const [form] = Form.useForm();
    const [dataBeginEdit, setdataBeginEdit] = useState<RoleTenantReadDto | undefined>(undefined);
    ...
    const _onOkModalAddOrEdit = () => {
        form.validateFields().then(async (values: RoleTenantReadDto) => {
            setloadingButton(true);
            if (isModalAdd) {
                let result = await services.CreateTeantRoleBasic({
                    id: 0,
                    displayName: values.displayName,
                    tenantId: props.idTenant,
                ...
                if (dataBeginEdit?.id) {
                    let result = await services.UpdateTeantRoleBasic({
                        id: dataBeginEdit.id,
                        displayName: values.displayName,
                        tenantId: props.idTenant,
                        isDefault: values.isDefault,
    ...
    const _onCancelModalAddOrEdit = () => {
        form.validateFields()
            .then((values: RoleTenantBasicDto) => {
                if (
                    values !== undefined &&
