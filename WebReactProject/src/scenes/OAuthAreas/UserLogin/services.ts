...
import { RegisterAccountDto, RegisterOutput } from './dtos/registerAccountDto';

declare var abp: any;

class LoginService {
    public async registerAccount(input: RegisterAccountDto)
        : Promise<ResponsesResource<RegisterOutput> | undefined> {
        try {
            let rs = await http.post('/api/services/app/Account/Register', input);
